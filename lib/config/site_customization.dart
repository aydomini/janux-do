...
class SiteCustomization {
  ...
  Color? matchAvatarGlow({
    String? username,
    String? primaryGroupName,
  }) {
    for (final rule in avatarGlowRules) {
      if (rule.primaryGroupName != null &&
          primaryGroupName == rule.primaryGroupName) {
        return rule.glowColor;
      }
      if (rule.username != null && username == rule.username) {
        return rule.glowColor;
      }
    }
    return null;
  ...
  Widget Function(String title, double fontSize)? matchTitleStyle({
    String? userTitle,
  }) {
    if (userTitle == null) return null;
    for (final rule in userTitleStyleRules) {
      if (userTitle == rule.title) {
        return rule.builder;
      }
    }
    return null;
