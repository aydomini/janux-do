...
class ErrorUtils {
  ...
  static ErrorInfo getErrorInfo(Object? error) {
    ...
    if (error is CloudflareChallengeException) {
      return ErrorInfo(
        icon: Icons.shield_rounded,
        title: 'JavBus 安全验证',
        message: 'JavBus 返回 Cloudflare 验证页面，请稍后重试或在浏览器确认访问状态。',
      );
    }
    if (error is ForumNetworkException) {
      return ErrorInfo(
        icon: Icons.signal_wifi_off_rounded,
        title: 'JavBus 网络连接失败',
        message: _forumMessage(error, '无法连接 JavBus，请检查网络、代理或站点访问状态。'),
      );
    }
    if (error is ForumResponseException) {
      return ErrorInfo(
        icon: Icons.public_off_rounded,
        title: 'JavBus 请求失败',
        message: _forumResponseMessage(error),
      );
    }
    if (error is ForumParseException) {
      return ErrorInfo(
        icon: Icons.data_object_rounded,
        title: 'JavBus 页面解析失败',
        message: _forumMessage(
          error,
          'JavBus 页面结构与当前解析器不匹配，需要更新解析规则或补充真实页面样本。',
        ),
      );
    }
    if (error is UnsupportedForumFeatureException) {
      return ErrorInfo(
        icon: Icons.block_rounded,
        title: '功能暂未支持',
        message: _forumMessage(error, '此 JavBus 功能不在第一阶段匿名浏览范围内。'),
      );
    }
    if (error is ForumException) {
      return ErrorInfo(
        icon: Icons.error_outline_rounded,
        title: 'JavBus 加载失败',
        message: _forumMessage(error, 'JavBus 数据加载失败。'),
      );
    }
  ...
  static String _forumMessage(ForumException error, String fallback) {
  ...
  static String _forumResponseMessage(ForumResponseException error) {
    final statusCode = error.statusCode;
    final message = _forumMessage(error, 'JavBus 返回了非预期响应。');
